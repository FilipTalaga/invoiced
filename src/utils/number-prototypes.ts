...
interface Number {
    ...
    toCurrency: (symbol?: { prefix?: string; suffix?: string; precision?: number }) => string;
    toPercent: () => string;
...
Number.prototype.toRounded = function (precision = 2) {
    return (Math.round(+this * Math.pow(10, precision)) / Math.pow(10, precision)).toFixed(
        precision,
    );
};
...
Number.prototype.toDigits = function (minDigits = 2) {
    ...
    return missingDigits > 0 ? `${new Array(missingDigits).fill('0').join('')}${digits}` : digits;
};
