...
const calculateInvoiceData = (entryInvoiceData: EntryInvoiceData): Invoice => {
    ...
    return {
        taxRatesSummary,
        total,
        invoiceNumber: `${invoiceNumber.toDigits()}/${moment(entryInvoiceData.dateOfIssue).format(
            'MM/YYYY',
        )}`,
        products: products.map(toInvoiceProduct),
        paymentDeadline: new Date(
            moment(entryInvoiceData.dateOfIssue).add(paymentDeadlineInDays, 'days').format(),
        ),
        ...exchangeProps,
        ...rest,
    };
